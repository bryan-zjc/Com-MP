...
class DetectorSlimHybridMAPPOEnv(HybridMAPPOEnv):
    ...
    def begin_control(self):
        ...
        self.previous_queue_ratio = defaultdict(float, {
            tls_id: self._reward_queue_level(tls_id)
    ...
    def _movement_queue_ratios(self, tls):
        """Eight canonical movement queue/capacity ratios, without averaging."""
        ...
        mask = self.topology_specs.get(
            tls, (np.ones(8, bool), np.ones(4, bool)))[0]
        ratios = np.zeros(8, dtype=float)
        for index, group in enumerate(lanes):
            if not mask[index]:
                continue
            unique_lanes = set(group)
            queue = sum(self._lane_value(lane, tc.LAST_STEP_VEHICLE_HALTING_NUMBER)
                        for lane in unique_lanes)
            capacity = sum(self.lane_capacity.get(lane, 1.0) for lane in unique_lanes)
            ratios[index] = queue / max(capacity, 1.0)
        return ratios

    def _reward_queue_level(self, tls):
        # Sum of differences equals difference of sums on the fixed template.
        return float(np.sum(self._movement_queue_ratios(tls)))

    def _accumulate_reward_step(self):
        for tls_id in self.net_info:
            queue_level = float(self._reward_queue_level(tls_id))
        ...
        for action, movements in enumerate(self.mp_tls_action_movements[tls]):
            ...
            for movement in movements:
                if not self._movement_allowed(tls, movement):
                    continue
                service = self._movement_service(movement, action)
                value = pressure_fn(movement, states, counts) * service
                lane = movement["in_lane"]
                # Choose one connection by service, not by clipped pressure.
                if lane not in by_lane or service > by_lane[lane][0]:
                    by_lane[lane] = (service, value)
            scores.append(sum(value for _, value in by_lane.values()))
        cache[key] = scores
        return scores
    ...
    def get_observation(self, tls):
        """22 slots: phase Q(4), phase CV(4), movement queues(8), phase(4), time, CV."""
        lanes, phases = self._template(tls)
        if tls not in self.topology_specs and (not all(lanes) or None in phases):
            self.set_topology_mask(tls)
        ...
        features = (pressures[0].tolist() + pressures[1].tolist() +
                    self._movement_queue_ratios(tls).tolist())
        current = int(traci.trafficlight.getPhase(tls) // 2)
