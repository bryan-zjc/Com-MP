...
class DetectorSlimHybridMAPPOEnv(HybridMAPPOEnv):
    ...
    def __init__(self, *args, queue_reduction_reward_weight=1.0,
        ...
        self.previous_queue_count = defaultdict(float)
    ...
    def begin_control(self):
        ...
        self.previous_queue_count = defaultdict(float, {
            tls_id: self._intersection_queue_count(tls_id)
            for tls_id in self.net_info
        })

    def _intersection_queue_count(self, tls_id):
        """Total halting vehicles on distinct controlled left/through lanes."""
        lanes = set(self.net_info[tls_id]["lanes_in_no_right"])
        return sum(float(self._lane_value(
            lane, tc.LAST_STEP_VEHICLE_HALTING_NUMBER)) for lane in lanes)

    def _accumulate_reward_step(self):
        for tls_id in self.net_info:
            queue_level = float(self._intersection_queue_count(tls_id))
            previous = float(self.previous_queue_count[tls_id])
            ...
            self.previous_queue_count[tls_id] = queue_level
