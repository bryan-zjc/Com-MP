...
if int(np.__version__.split(".", 1)[0]) >= 2:
    raise RuntimeError(
        "This training environment requires NumPy < 2 because the installed "
        "PyTorch/SciPy stack was compiled against the NumPy 1.x ABI. "
        "Install numpy==1.26.4 and restart the Python/Spyder kernel.")
...
def make_env(config, episode_seed, tripinfo_path):
    ...
    env = DetectorSlimHybridMAPPOEnv(
        str(route_file), str(config.net_file), len(tls_ids),
        ...
        stop_exactly_at_max_steps=True,
    )
...
def init_wandb(config, device):
    ...
    if not api_key:
        print(
            "WANDB_API_KEY is not set; training continues without W&B. "
            "Set your own key in the environment or use --no-wandb.")
        return None
...
def parse_args():
    ...
    parser.add_argument("--cv-rate", type=float, default=0.8)
    parser.add_argument(
        "--experiment-name", default="com_mp_5x5_d15k_cv08")
...
if __name__ == "__main__":
    ...
    print(f"Route file: {cfg.rou_file}")
    print(f"Episode horizon: {cfg.max_steps}s")
