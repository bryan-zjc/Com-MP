...
class HybridMAPPOEnv(HybridMPEnv):
    ...
    def __init__(self, *args, alpha_bins, queue_reward_weight=1.0,
                 ...
                 reward_scale=0.01, state_waiting_scale=300.0,
    ...
    def reset(self, tripinfo_path):
        ...
        self.max_actions = max(
            4,
            max(info["phase_num"] // 2 for info in self.net_info.values()),
        )
    ...
    def _refresh_lane_cache(self):
        ...
        self.lane_cache = self._lane_subscription_cache
    ...
    def _collect_mp_movement_states(self):
        ...
        for movement in self.mp_all_movements:
            ...
            self.mp_movement_states[movement["key"]] = (
                count / self.mp_lane_lengths[movement["in_lane"]])
    ...
    def _select_phase_by_alpha(self, tls_id, alpha):
        """Select the phase with the largest composite pressure."""
        pressure = np.asarray(
    ...
    def step(self, alpha_action):
        ...
        for tls_id in self.net_info:
            if self.decision_step[tls_id] != current_time:
                continue
            current_action = int(self._current_phase(tls_id) // 2)
            alpha = float(alpha_action.get(tls_id, self.last_alpha[tls_id]))
            forced_action = self._max_red_action(tls_id, current_time)
            selected_action = (
                forced_action if forced_action is not None
                else self._select_phase_by_alpha(tls_id, alpha))
            if selected_action != current_action:
                self.pending_switch_events[tls_id] += 1
        return super().step(alpha_action)
    ...
    def get_observation(self, tls_id):
        ...
        tail = current_one_hot + [min(elapsed / max(self._max_red, 1), 1.5),
    ...
    def get_action_diagnostics(self, tls_id):
        ...
        phases = [int(np.argmax(alpha * cv + (1.0 - alpha) * mp))
                  for alpha in self.alpha_bins]
        forced_action = self._max_red_action(
            tls_id, self._current_time())
        if forced_action is not None:
            phases = [forced_action for _ in phases]
        return {
