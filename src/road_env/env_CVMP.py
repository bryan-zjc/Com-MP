...
class Intersec_Env(MP_Intersec_Env):
    ...
    def step(self, action):
        ...
        for tls_id in self.net_info.keys():
            ...
            if decision_now and forced_action is not None:
    ...
    def _enter_cv_absence_fallback(self, tls_id, current_time):
        ...
        if current_phase % 2 == 1:
            ...
            self._tls_subscription_cache.setdefault(tls_id, {})[
                tc.TL_CURRENT_PHASE] = current_action * 2
        else:
            current_action = (
    ...
    def _resolve_cv_absence_fallback(
        ...
        if (decision_now and
            ...
            self._tls_subscription_cache.setdefault(tls_id, {})[
                tc.TL_CURRENT_PHASE] = target_phase
            self.phase_switch_pointer[tls_id] = current_time
    ...
    def _update_cv_vehicle_states(self):
        ...
        for lane_id in self.turn_tracked_in_lanes:
            for veh_id in self._lane_value(
                    lane_id, tc.LAST_STEP_VEHICLE_ID_LIST, ()):
                if self._is_cv_vehicle(veh_id):
                    self.active_cv_ids.add(veh_id)
        ...
        contributing_ids = {
            veh_id for veh_id, movement_key in
            self.vehicle_movement_key.items() if movement_key is not None
        }
        for veh_id in contributing_ids.difference(relevant):
    ...
    def _dynamic_downstream_state(self, movement, movement_states,
        ...
        # Q-MP and CV-MP share the same realized local turning-flow ratios.
    ...
    def _collect_pressure(self, tls_id):
        ...
        for action_idx, phase_movements in enumerate(
                self.tls_action_movements[tls_id]):
            service_factor = self._phase_service_factor(
                tls_id, action_idx)
            phase_pressure = 0.0
            unconstrained_phase_pressure = 0.0
            for movement in phase_movements:
                movement_pressure = self._movement_pressure(
                    movement, movement_states, movement_counts)
                unconstrained_phase_pressure += (
                    movement_pressure * self.saturation_flow_rate *
                    service_factor)
                phase_pressure += (
                    movement_pressure *
            ...
            phase_serviceable = phase_pressure > 1e-9
            raw_pressure.append(unconstrained_phase_pressure)
