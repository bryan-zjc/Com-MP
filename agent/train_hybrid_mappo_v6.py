...
# Optional Weights & Biases integration reads the user's own key from the
# WANDB_API_KEY environment variable. Never commit a personal API key here.

# Change this value to select the default training network.
# The command-line option --intersection can still override it.
TRAIN_INTERSECTION = "qinzhou"  # Supported values: "5x5", "qinzhou"
SUPPORTED_INTERSECTIONS = ("5x5", "qinzhou")
...
def make_env(config, episode_seed, tripinfo_path):
    ...
    env = DetectorSlimHybridMAPPOEnv(
        ...
        pressure_switch_margin=config.pressure_switch_margin,
...
def init_wandb(config, device):
    ...
    if not api_key:
        print(
            ...
            "Set it to your own key and pass --wandb to enable logging."
        )
        return None
...
def parse_args():
    ...
    parser.add_argument(
        "--experiment-name", default="mappo_v6_detector_slim_10s")
...
if __name__ == "__main__":
    ...
    cfg = ExperimentConfig(experiment_name=args.experiment_name,
                           ...
                           pressure_switch_margin=args.switch_margin,
