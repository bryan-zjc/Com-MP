...
class HybridMAPPOEnv(HybridMPEnv):
    ...
    def __init__(self, *args, alpha_bins, queue_reward_weight=1.0,
                 ...
                 reward_scale=0.01, pressure_switch_margin=0.0,
        ...
        self.pressure_switch_margin = float(pressure_switch_margin)
    ...
    def _collect_mp_movement_states(self):
        ...
        for movement in self.mp_all_movements:
            ...
            self.mp_movement_states[movement["key"]] = count
    ...
    def _select_phase_by_alpha(self, tls_id, alpha):
        """Apply a small pressure hysteresis to avoid near-tie phase chatter."""
        pressure = np.asarray(
        ...
        return self._select_phase_with_margin(tls_id, pressure)

    def _select_phase_with_margin(self, tls_id, pressure):
        selected_action = int(np.argmax(pressure))
        current_action = int(traci.trafficlight.getPhase(tls_id) // 2)
        if (selected_action != current_action and
                current_action < len(pressure) and
                pressure[selected_action] - pressure[current_action] <
                self.pressure_switch_margin):
            return current_action
    ...
    def step(self, alpha_action):
        ...
        for tls_id in self.net_info:
            if self.decision_step[tls_id] != current_time:
                continue
            current_action = int(traci.trafficlight.getPhase(tls_id) // 2)
            at_max_green = current_time >= (
                self.phase_switch_pointer[tls_id] + self._green_max)
            if at_max_green or self.g_max_flag[tls_id] == 1:
                continue
            alpha = float(alpha_action.get(tls_id, self.last_alpha[tls_id]))
            selected_action = self._select_phase_by_alpha(tls_id, alpha)
    ...
    def get_observation(self, tls_id):
        ...
        tail = current_one_hot + [min(elapsed / max(self._green_max, 1), 1.5),
    ...
    def get_action_diagnostics(self, tls_id):
        ...
        phases = [self._select_phase_with_margin(
            tls_id, alpha * cv + (1.0 - alpha) * mp)
                  for alpha in self.alpha_bins]
        current_action = int(traci.trafficlight.getPhase(tls_id) // 2)
        at_max_green = traci.simulation.getTime() >= (
            self.phase_switch_pointer[tls_id] + self._green_max)
        if at_max_green:
            forced_action = (current_action + 1) % len(mp)
            phases = [forced_action if phase == current_action else phase
                      for phase in phases]
        return {
